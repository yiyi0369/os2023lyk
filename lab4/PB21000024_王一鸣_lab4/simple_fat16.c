...
cluster_t read_fat_entry(cluster_t clus)
{
    ...
    /** Your Code Here ... **/
    //一个clus item 对应2个字节
    size_t fat_offset = clus << 1;
    sector_t fat_sec = meta.fat_sec + (fat_offset / meta.sector_size);
    sector_read(fat_sec, sector_buffer);
    size_t offset = fat_offset % meta.sector_size;
    return *((cluster_t *)&sector_buffer[offset]);
    
}
/*
void exam_fat(cluster_t clus){
    //printf("exam_fat\n");
//printf("\tstart\n");
    //printf("\tfrom cluster %d\n",clus);
    while(is_cluster_inuse(clus)){
        //printf("\tcluster %d is in use\n",clus);
...
int find_entry_in_sectors(const char* name, size_t len, 
    ...
    for(size_t i = 0; i < sectors_count; i++) {
        ...
        // 2. 对该扇区中的每一个###目录项，检查是否是待查找的目录项（注意检查目录项是否合法）
        int ret=sector_read(from_sector+i,buffer);
        if(ret)
        {
            printf("\tfailed to read the sector\n");
            return -1;
        }
    ...
    while (**remains != '\0' && state == FIND_EXIST) {
        ...
        if(level == 0) {
            ...
            while (is_cluster_inuse(clus)) {    // 依次查找每个簇
                ...
                state = find_entry_in_sectors(*remains,len,sec,nsec,slot);
                if(state < 0) { // 出现错误
                    return state;
...
void *fat16_init(struct fuse_conn_info * conn, struct fuse_config *config) {
    ...
    BPB_BS bpb;
    sector_read(0, &bpb);
    //先从0号扇区读取BPB信息，这个信息是固定的，不会变的，所以只需要读取一次
    //什么是BPB
        ...
        for(size_t i=0; i < nsec; i++) {
            ...
            for (size_t off = 0; off < meta.sector_size; off += DIR_ENTRY_SIZE)
            {
                ...
                else 
                {
                    ...
                    //printf("\tdir->DIR_Attr: %d\n", dir->DIR_Attr);
                    //printf("\tdir->DIR_FstClusLO: %d\n", dir->DIR_FstClusLO);
                    //printf("\tdir->DIR_FileSize: %d\n", dir->DIR_FileSize);
                    char longname[MAX_NAME_LEN];
                    //exam_fat(dir->DIR_FstClusLO);
                    to_longname(dir->DIR_Name, longname, MAX_NAME_LEN);
                    //printf("\tlongname: %s\n", longname);
                    filler(buf, longname, NULL, 0, 0);
                }
            }
        }
...
int fat16_read(const char *path, char *buffer, size_t size, off_t offset,
   ...
   // printf("\tread(path='%s', offset=%ld, size=%lu)\n", path, offset, size);
    ...
    /*
    char *sector_buffer=malloc(meta.cluster_size);
    size_t sector_offset = offset;
    while(sector_offset >= meta.cluster_size){
        clus = read_fat_entry(clus);
        sector_offset -= meta.cluster_size;
        if(is_cluster_end(clus))
            return 0;
    }
    size_t sector_size = min(size, meta.cluster_size - sector_offset);
    while(p < size){
        size_t len = read_from_cluster_at_offset(clus, sector_offset, sector_buffer, sector_size);
        memcpy(buffer + p, sector_buffer + sector_offset, len);
        p += len;
        sector_offset = 0;
        sector_size = min(size - p, meta.cluster_size);
        clus = read_fat_entry(clus);
        if(is_cluster_end(clus))
            break;
    }
    */
    ...
    {
        //处理每次读取的簇
        //分两种情况，一种是读取的数据在一个簇内，一种是跨簇
        //簇内读取
        cnt++;
        //printf("\tcnt=%d\n", cnt);
        
        if(offset/meta.cluster_size == (offset+size-1-p)/meta.cluster_size)
        {
            
            //printf("\nlast cluster\n");
            size_t len = read_from_cluster_at_offset(clus, offset, sector_buffer, size-p);
            memcpy(buffer + p, sector_buffer, len);
            p += len;
            break;
        }//跨簇读取
        else
        {
            //printf("\tnot last cluster\n");
            //printf("\tp=%lu\n", p);
            size_t len = read_from_cluster_at_offset(clus, offset, sector_buffer, meta.cluster_size-offset);
            //printf("\tlen=%lu\n", len);
            memcpy(buffer + p, sector_buffer, len);
            //printf("\tmemcpy\n");
            p += len;
...
int write_fat_entry(cluster_t clus, cluster_t data) {
    ...
    for(size_t i = 0; i < meta.fats; i++) {
        ...
        //   2. 读取该扇区并在对应位置写入数据
        //   3. 将该扇区写回
        sector_t fat_sec = meta.fat_sec + meta.sec_per_fat * i;
        sector_t sec = fat_sec + clus_sec;
        sector_read(sec, sector_buffer);
        //printf("check write fat1");
        memcpy(sector_buffer + sec_off, (&data), sizeof(cluster_t));
        //*(cluster_t *)(sector_buffer+sec_off)=data;
        //printf("check write fat2");
 ...
 * @param fat16_clus 文件系统指针
 * @param n         要分配簇的个数
 * @return int      成功返回0，失败返回错误代码负值
 */
int alloc_clusters(size_t n, cluster_t* first_clus) {
    //printf("alloc_clusters\n");
    //随机初始化一个簇号
    static cluster_t search_clus=0x0002;
    if (n == 0)
        return CLUSTER_END;
    ...
    //printf("\tcheck 1\n");
    // TODO:2.3: 扫描FAT表，找到n个空闲的簇，存入cluster数组。注意此时不需要修改对应的FAT表项。
    // Hint: 你可以使用 read_fat_entry 函数来读取FAT表项的值，根据该值判断簇是否空闲。
    cluster_t clus = search_clus;
    while(allocated < n){
        //printf("\t %d\n",clus);
        cluster_t fat_clus = read_fat_entry(clus);
        if(is_cluster_end(clus))
            break;
        if(fat_clus == CLUSTER_FREE){
    ...
    // TODO:2.4: 修改clusters中存储的N个簇对应的FAT表项，将每个簇与下一个簇连接在一起。同时清零每一个新分配的簇。
    // 清零要分配的簇
    for(size_t i = 0; i < n; i++) {
        //printf("\tclus[%d] = %u",i,clusters[i]);
    ...
    //printf("\tcheck 3\n");
    // TODO:2.5: 连接要分配的簇的FAT表项（Hint: 使用write_fat_entry）
    // Hint: 将每个簇连接到下一个即可
    for(size_t i = 0; i < n; ++ i){
...
int fat16_mkdir(const char *path, mode_t mode) {
    ...
    DirEntrySlot slot,slot_1,slot_2;
    const char* filename= NULL;
    //printf("\tfuck you 1\n");
    int ret = find_empty_slot(path, &slot, &filename);
    if(ret < 0) {
    ...
    cluster_t firstclus[2];
    alloc_clusters(1,firstclus);
    ret = dir_entry_create(slot, shortname, 0x10, firstclus[0],2*sizeof(DIR_ENTRY));
    //printf("\n\tfist clus=%u\n",firstclus[0]);
    char newpath[200]="";
    char newpath_1[200]="";
    char newpath_2[200]="";
    strcat(newpath,path);
    strcat(newpath,"/");
    strcat(newpath_1,newpath);
    strcat(newpath_1,DOT_NAME);
    strcat(newpath_2,newpath);
    strcat(newpath_2,DOTDOT_NAME);
    find_empty_slot(newpath_1, &slot_1,&filename);
    dir_entry_write(slot_1);
    find_empty_slot(newpath_2, &slot_2,&filename);
    dir_entry_write(slot_2);
    /*
    printf("\nnewpath1=%s\n",newpath_1);
    printf("\nnewpath2=%s\n",newpath_2);
    find_empty_slot(newpath_1, &slot_1,&filename);
...
int fat16_rmdir(const char *path) {
    ...
    if (root)
    {
        ...
        sector_t nsec = meta.root_sectors;
        // 把根目录的所有簇都设置为空闲
        for (size_t i = 0; i < nsec; i++)
        {
            sector_t sec = first_sec + i;
            const char sec_buffer[MAX_LOGICAL_SECTOR_SIZE] = {0};
            sector_write(sec, sec_buffer);
        }
        return 0;
    }
    DirEntrySlot slot;
    ...
    while (is_cluster_inuse(clus))
    {
        ...
        for (size_t i = 0; i < nsec; i++)
        {
            ...
            {
                DIR_ENTRY *dir = (DIR_ENTRY *)(sector_buffer + off);
                if (check_name(DOT_NAME, strlen(DOT_NAME), dir->DIR_Name)&&flag1==0)
                {
                    flag1 = 1;
                    continue;
                }else if( check_name(DOTDOT_NAME, strlen(DOTDOT_NAME), dir->DIR_Name)&&flag2==0)
                {
                    flag2 = 1;
                    continue;
                }
 ...
 * @return size_t  成功写入的字节数，失败返回错误代码负值。可能部分成功，此时仅返回成功写入的字节数，不提供错误原因（POSIX标准）。
 */
ssize_t write_to_cluster_at_offset(cluster_t clus, off_t offset, const char* data, size_t size) {
    //printf("write_to_cluster_at_offset\n");
    assert(offset + size <= meta.cluster_size);  // offset + size 必须小于簇大小
    char sector_buffer[PHYSICAL_SECTOR_SIZE];
...
int file_reserve_clusters(DIR_ENTRY* dir, size_t size) {
    ...
    cluster_t clus=dir->DIR_FstClusLO;
    cluster_t last_clus=clus;
    size_t need = (size + meta.cluster_size - 1) / meta.cluster_size;
    int cnt = 0;
    while (is_cluster_inuse(clus))//找到最后一个簇
    {
        cnt ++;
        last_clus = clus;
        clus = read_fat_entry(clus);
    ...
    if(cnt>=need)
    {
        ...
        cluster_t newclus[1];
        int ret =alloc_clusters(need-cnt, newclus);
        if(ret < 0) {
            //printf("\t check point 4\n");
            //printf("alloc_clusters failed\n");
            return ret;
        }
        if(!is_cluster_inuse(dir->DIR_FstClusLO))
        {
            dir->DIR_FstClusLO = newclus[0];
            //printf("\t check point 2\n");
            return 0;

        }    
        //printf("\t check point 3\n");
        write_fat_entry(last_clus, newclus[0]);
    }
...
int fat16_write(const char *path, const char *data, size_t size, off_t offset,
    ...
    file_reserve_clusters(dir, size + offset);
    slot.dir.DIR_FileSize = max(slot.dir.DIR_FileSize, offset + size);
    ret = dir_entry_write(slot);
...
int fat16_truncate(const char *path, off_t size, struct fuse_file_info* fi) {
    ...
    else if(size>dir->DIR_FileSize)
    {
        size_t p = dir->DIR_FileSize;
        char *data = (char*)malloc(size-p);
        memset(data, 0, size-p);
        fat16_write(path, data, size-p, p, fi);
        free(data);
        return 0;
    }else{//dir->DIR_FileSize>size一定大于0
        //两步
        //1.释放多余的簇
        //2.修改文件大小
        cluster_t clus = dir->DIR_FstClusLO;
        int cnt = 0;
        size_t p = meta.cluster_size;
        while(size >= p) {
            clus = read_fat_entry(clus);
            p += meta.cluster_size;
        }
        free_clusters(read_fat_entry(clus));
        write_fat_entry(clus, 0x0FFFF);
