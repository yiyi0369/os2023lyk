...
// 定义要输出的文件
#define OUTPUT_FILE "/home/wym_linux/oslab/lab3/lab3_2/expr_result.txt"
...
static my_page_referenced mpage_referenced = (my_page_referenced)0xffffffffa72f1590;
// sudo cat /proc/kallsyms | grep follow_page
static my_follow_page mfollow_page = (my_follow_page)0xffffffffa72d34e0;
...
static void scan_vma(void)
{
    ...
    if (mm)
    {
        ...
        struct vm_area_struct *vma_ptr = mm->mmap;
        while (vma_ptr != NULL)
        {
...
static void print_mm_active_info(void)
{
    ...
    if(IS_ERR_OR_NULL( my_task_info.task))
    {
        printk("something goes wrong...");
        return;
    }    
    struct mm_struct *mm = get_task_mm(my_task_info.task);
    ...
    if ((mm))
    {
        ...
        while ((vma_ptr))
        {   
            unsigned long start = vma_ptr->vm_start;
            unsigned long end = vma_ptr->vm_end;
            ...
            {
                printk("DEBUG_2");
                struct page *vma_page = mfollow_page(vma_ptr, start, FOLL_GET);
                if(!IS_ERR_OR_NULL(vma_page)&&!IS_ERR_OR_NULL((struct mem_cgroup *)(vma_page->memcg_data)))
                {
                    printk("debug_3");

                    unsigned long vm_flags;
                    int active = mpage_referenced(vma_page, 0,(struct mem_cgroup *)(vma_page->memcg_data), &vm_flags);
                    printk("active=%d",active);
                }
           ...
           */
            ...
            while (addr < end)
            {
                struct page *vma_page = mfollow_page(vma_ptr, addr, FOLL_GET);
                unsigned long vm_flags;
                printk(" addr+%d;",i);
                i++;
                
                if (!IS_ERR_OR_NULL(vma_page) )
                {
                    int active = mpage_referenced(vma_page, 0, (struct mem_cgroup *)(vma_page->memcg_data), &vm_flags);
                    if (active >= 1)
                    {
...
static unsigned long virt2phys(struct mm_struct *mm, unsigned long virt)
{
    ...
    if(IS_ERR_OR_NULL(mm)||IS_ERR_OR_NULL(mm->mmap))
    {
    ...
    if(IS_ERR_OR_NULL(mm->pgd)){
        pr_err("func: %s page is NULL\n", __func__);
        return 0;
    }else{
        pgd_t *pgd_offset_0=pgd_offset(mm,virt);
        if(IS_ERR_OR_NULL(pgd_offset_0)||pgd_none(*pgd_offset_0)||pgd_bad(*pgd_offset_0))
        {pr_err("func: %s page is NULL\n", __func__);
            return 0;
        }
        pud_t *pud_offset_0=pud_offset((p4d_t*)pgd_offset_0,virt);
        if(IS_ERR_OR_NULL(pud_offset_0)||pud_none(*pud_offset_0)||pud_bad(*pud_offset_0))
        {
            pr_err("func: %s page is NULL\n", __func__);
            return 0;
        }
        pmd_t *pmd_offset_0=pmd_offset(pud_offset_0,virt);
        if(IS_ERR_OR_NULL(pmd_offset_0)||pmd_none(*pmd_offset_0)||pmd_bad(*pmd_offset_0))
        {
            pr_err("func: %s page is NULL\n", __func__);
            return 0;
        }
        pte_t *pte_offset_0=pte_offset_map(pmd_offset_0,virt);
        if(IS_ERR_OR_NULL(pte_offset_0)||pte_none(*pte_offset_0))//||pte_bad(*pte_offset_0)
        {
            pr_err("func: %s page is NULL\n", __func__);
            return 0;
        }
        return pte_pfn(*pte_offset_0);
    }
    /*
    struct page *page = mfollow_page(mm->mmap, virt, FOLL_GET);//直接使用函数进行映射
...
static void traverse_page_table(struct task_struct *task)
{
    ...
    if (!IS_ERR_OR_NULL(mm))
    {
        ...
        {   
            ...
            while (addr < end)
            {
                struct page *page = mfollow_page(vma_ptr, addr, FOLL_GET);
                unsigned long vm_flags;
                if (!IS_ERR_OR_NULL(page))
                {
                    ...
                    record_two_data(addr,virt2phys(mm,addr));
                    record_two_data(addr,page_to_pfn(page));
                }
                addr += PAGE_SIZE;
            }
            ...
            while (addr < end)
            {
                ...
                if (!IS_ERR_OR_NULL(page))
                {
                    ...
                    if(IS_ERR_OR_NULL(ptr))
                        return;
                    //virt2phys(mm,addr);
                    //memcpy(str_buf,ptr,1);
                    int i=0;
                    while(i<PAGE_SIZE)//*((char*)ptr+i)!=125&&
                    {
                        if(*((char*)ptr+i)>=32&&*((char*)ptr+i)<=127)
                        {    printk(KERN_CONT"%c",*((char*)ptr+i));
                            buf[curr_buf_length]=*((char*)ptr+i);
                            curr_buf_length++;
                        }
                        i++;
                    }
                    buf[curr_buf_length]=0;
                    printk("\n");
                    flush_buf(0);
                    //flush_buf(0);
                    kunmap_atomic(ptr);
