...
#define HDRP(bp) ((char *)(bp)-WSIZE)                        /*bp point to the alloc_pointer*/
...
void mm_free(void *bp)
{
    ...
    //printf("%s, addr_start=%u, size_head=%u, size_foot=%u\n",
        //"mm_free", HDRP(bp), (size_t)GET_SIZE(HDRP(bp)), (size_t)GET_SIZE(FTRP(bp)));
...
{
    //printf("realloc\n");
    //printf("\n in realloc : size=%u,ptr=%u", size,ptr);
    void *oldptr = ptr;
    void *newptr;
    size_t copySize;
...
static void *coalesce(void *bp)
{
    ...
    */
    ...
    else if (prev_alloc && !next_alloc) /*前块已分配，后块空闲*/
    {
        ...
        char *p_nbr_h=HDRP(NEXT_BLKP(bp));
        char *p_nbr_f=FTRP(NEXT_BLKP(bp));
        delete_from_free_list(NEXT_BLKP(bp));
        add_to_free_list(bp);
        PUT(FTRP(bp), 0);
        PUT(HDRP( bp), PACK(size + size_next, 1, 0)); /*change the block size*/
        PUT(p_nbr_h,0);
        PUT(p_nbr_f,PACK(size + size_next, 1, 0));
    ...
    else if (!prev_alloc && next_alloc) /*前块空闲，后块已分配*/
    {
        ...
        char *p_nbr_h=HDRP(PREV_BLKP(bp));
        char *p_nbr_f=FTRP(PREV_BLKP(bp));
        char *bp_new=PREV_BLKP(bp);
        /*next_block*/
        /*succ_block*/
        PUT(p_nbr_f, 0);
        PUT(p_nbr_h, PACK(size + size_prev, 1, 0)); /*change the block size*/
        PUT(FTRP(bp),PACK(size + size_prev, 1, 0));
        PUT(HDRP(bp),0);
        //mm_check("coace");
        //printf("???????");
        return bp_new;
    ...
    else /*前后都是空闲块*/
    {
        ...
        char *p_prev_h=HDRP(bp_prev);
        char *p_prev_f=FTRP(bp_prev);
        char *p_next_h=HDRP(bp_next);
        char *p_next_f=FTRP(bp_next);
        
        PUT(p_prev_h,PACK(size+size_next+size_prev,1,0));
        PUT(p_next_f,PACK(size+size_next+size_prev,1,0));
        PUT(HDRP(bp),0);
        PUT(FTRP(bp),0);
        PUT(p_prev_f,0);
        PUT(p_next_h,0);
        delete_from_free_list(bp_next);
        //printf("\nnot alocated pre next\n");
        return bp_new;
    }
    return bp;
}
...
static void *find_fit_first(size_t asize)
{
    //printf("\n in find_fit_first : size=%lu", asize);
    /*
        首次匹配算法
        TODO:
    ...
    */
    char*p_start=free_listp;
    while(p_start!=NULL)
    {
        if(GET_SIZE(HDRP(p_start))>=asize)
        {
            //printf("\nfind BP=%u",p_start);
            return p_start;
        }
        p_start=(char*)GET_SUCC(p_start);
    }
    //printf("\nfind BP=NULL");
    return NULL; // 换成实际返回值
}
...
static void *find_fit_best(size_t asize)
{
   ...
   while(p_start!=NULL)
   {
    if((GET_SIZE(HDRP(p_start))>=asize)&&(min==0||GET_SIZE(HDRP(p_start))<min))
    {
        min=GET_SIZE(HDRP(p_start));
        p_best=p_start;
    }
    p_start=(char*)GET_SUCC(p_start);
   }
...
static void place(void *bp, size_t asize)
{
   ...
   size_t size=GET_SIZE(HDRP(bp));
   int pre_alloc=GET_PREV_ALLOC(HDRP(bp));
    //printf("\nin place: aszie=%u,bp=%u\n",asize,bp);
   //mm_check("place");
   //
   
   if((int)size-(int)asize<MIN_BLK_SIZE)/*change the type of it*/
   {
        char*bp_next=NEXT_BLKP(bp);
        char*head_next=HDRP(bp_next);
        char*feet_next=FTRP(bp_next);
        PUT(head_next,PACK(GET_SIZE(head_next),1,GET_ALLOC(head_next)));
        PUT(feet_next,PACK(GET_SIZE(head_next),1,GET_ALLOC(head_next)));
        
        PUT(HDRP(bp),PACK(size,pre_alloc,1));
        ...
        //change the head to the used_end
        delete_from_free_list(bp);
        //printf("\nSIZE=%u\n",size);
        PUT(FTRP(bp),PACK(size-asize,1,0));
        PUT(HDRP(bp),PACK(asize,pre_alloc,1));
        PUT(HDRP(bp)+asize,PACK(size-asize,1,0));
        add_to_free_list((char*)bp+asize);
...
{
    //if(strcmp(function,"place_end")!=0)
        //return;
    ...
    /**/ char *bp = free_listp;
     ...
     {
        //if(GET_SIZE(HDRP(bp))>=4000000000)
         //exit(-1);
         count_empty_block++;
