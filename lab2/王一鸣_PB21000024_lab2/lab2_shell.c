...
int process_redirect(int argc, char **argv, int *fd) // change the direction of pipefd(sub process)
{
    ...
    while (i < argc)
    {
        ...
        {
            // TODO: 打开输出文件从头写入
            file = open(argv[i + 1], O_WRONLY | O_CREAT | O_TRUNC, S_IRWXU);
            if (file < 0)
            {
                printf("open '%s' error\n", argv[i + 1]);
            }
        ...
        {
            // TODO: 打开输出文件追加写入
            file = open(argv[i + 1], O_WRONLY | O_APPEND | O_CREAT, S_IRWXU);
            if (file < 0)
            {
                printf("open '%s' error\n", argv[i + 1]);
            }
        ...
        {
            // TODO: 读输入文件
            file = open(argv[i + 1], O_RDONLY, S_IRWXU);
            if (file < 0)
            {
                printf("open '%s' error\n", argv[i + 1]);
            }
...
int execute(int argc, char **argv)
{
    ...
    execlp(argv[0], argv[0], argv[1], argv[2], argv[3], argv[4], argv[5], argv[6], argv[7], argv[8], argv[9],
           ...
           argv[23], argv[24], argv[25], argv[26], argv[27], argv[28], argv[29], argv[30], argv[31], NULL); // execute one by one
    return 0;
}
...
{
    /* 输入的命令行 */
    char cmd_string[MAX_CMDLINE_LENGTH];//string

    char *commands_pipe[128];//cmds after pipe
    int cmd_cnt_pipe;
    ...
    while (1)
    {
        ...
        printf("shell:%s -> ", CWD);
        fflush(stdout);
        memset(cmd_string, 0, MAX_CMDLINE_LENGTH);
        fgets(cmd_string, 256, stdin);
        strtok(cmd_string, "\n");

        /* TODO: 基于";"的多命令执行，请自行选择位置添加 */
        char *cmds[MAX_CMD_ARG_NUM] = {0};
        cmd_cnt = split_string(cmd_string, ";", cmds);
        /* 由管道操作符'|'分割的命令行各个部分，每个部分是一条命令 */
        /* 拆解命令行 */
        for (int k = 0; k < cmd_cnt; k++)
        {
            cmd = cmds[k]; //
            cmd_cnt_pipe = split_string(cmd_string, "|", commands_pipe);

            if (cmd_cnt_pipe == 0)
            {
                continue;
            }
            else if (cmd_cnt_pipe == 1)
            { // 没有管道的单一命令
            ...
            { // 两个命令间的管道
                ...
                if (pid == 0)
                {
                    ...
                    close(pipefd[READ_END]);                // the read_end initially point to the pipe_read,needed to be closed
                    dup2(pipefd[WRITE_END], STDOUT_FILENO); //redirect the stdout to the write_end,which means if a file initially write to the display,\
                    \it write to the write_end
                    close(pipefd[WRITE_END]);               //the initial end can be closed,its used as a pointer where the pipe end is\
                    \if not closed it will influence the in/out put
                    /*
                    ...
                    int argc = split_string(commands_pipe[0], " ", argv);
                    execute(argc, argv);
                    exit(255);
                }
                ...
                if (pid == 0)
                {
                     ...
                     *
                     */
                    int argc = split_string(commands_pipe[1], " ", argv);
                    execute(argc, argv);
                    exit(255);
                }
                close(pipefd[WRITE_END]);
            ...
            {                // 选做：三个以上的命令
                ...
                for (int i = 0; i < cmd_cnt_pipe; i++)
                {
                     ...
                     *
                     */
                    // first create pipe then redirect the pipe in/out
                    int ret;
                    if (i < cmd_cnt_pipe - 1)
                    {
                        ret = pipe(pipefd);
                        ...
                        ret = 1; // 0 i-1 create pipe or failed
                    ...
                    if (pid == 0)
                    {
                        ...
                        }                         // if not the last commond ,redirect to the std_out
                        close(pipefd[WRITE_END]); //
                        ...
                        int argc = split_string(commands_pipe[i], " ", argv);
                        execute(argc, argv);
                        exit(255);
                    }
                    if (i > 0)
                    ...
                    /* TODO:父进程除了第一条命令，都需要关闭当前命令用完的上一个管道读端口
