...
SYSCALL_DEFINE4(my_top, int __user *, pid, char __user *, temp, int __user *, isrunning, unsigned long long __user *, cputime)
{
	...
	int cnt = 0;
	int j = 0;
	int id[process_size] = {0};
	char name[process_size * name_size] = {0};
	unsigned long long cpu[process_size] = {0};
	int isrun[process_size] = {0};
	...
	for_each_process(task)
	{
		...
		for (j = 0; j < name_size; j++)
		{
			...
			if (flag == 0)
			{
				name[name_size * cnt + j] = '\0';
			}
			else
			{
				name[name_size * cnt + j] = task->comm[j];
			}
		}
		cpu[cnt] = task->se.sum_exec_runtime;
		if(task->state == 0){
			isrun[cnt]=1;
		}else{
			isrun[cnt]=0;
